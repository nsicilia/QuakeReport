...
public final class QueryUtils {
    ...
    public static ArrayList<Quakes> extractEarthquakes() {
        ...
        try {
            ...
            //This parses the initial string and creates a JSON object
            JSONObject root = new JSONObject(SAMPLE_JSON_RESPONSE);
            //This pulls an array from the object using the name tag in the JSON
            JSONArray featuresArray =  root.getJSONArray("features");

            for (int i = 0; i < featuresArray.length(); i++) {
                //This creates the JSON object that was incapacitated in the array
                JSONObject object = featuresArray.getJSONObject(i);

                //This gets the object that was nested within the properties tag
                JSONObject list = object.getJSONObject("properties");

                //Pulls the magnitude info from "mag" tag
                double magItem = list.getDouble("mag");

                //Pulls the location infor from the "place" tag
                String locationItem = list.getString("place");
